...
from diffusion.nn import EmbeddingType, DownBlockType, UpBlockType, UpSampleMode, ResnetTimeScaleShiftType, get_embedding, \
    get_down_block, get_up_block, MidBlock, Projector, NormalizationType
...
class DualEncoderUNetModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
            ...
            *,
            ...
            down_block_types_branch_2: Tuple[DownBlockType | str, ...] = ("ResnetDownBlock", "ResnetDownBlock", "AttnDownBlock", "AttnDownBlock"),
            ...
            block_out_channels_2: Tuple[int | str, ...] = (64, 128, 256, 512),
        ...
        self.down_blocks_branch_2 = nn.ModuleList([])
        ...
        for i, down_block_type in enumerate(down_block_types_branch_2):
            ...
            self.down_blocks_branch_2.append(down_block)
    ...
    def initialize_projectors(
        ...
        for i, channels in enumerate(self.block_out_channels_2):
    ...
    def forward(
        ...
        down_block_res_samples_2 = (sample_2,)
        for i, downsample_block in enumerate(self.down_blocks_branch_2):
            if hasattr(downsample_block, "skip_conv"):
        ...
        sample_2 = self.projectors[-1].fbp_channel_wise_scaled(sample_2)
        # 4. mid
        sample = self.mid_block(torch.cat([sample, sample_2], dim=1), embedding_sum)

        current_size = sample_2.size(-1)
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            combined_res_samples = []

            for (s, s2) in zip(res_samples, res_samples_2):

                if s2.size(-1) == current_size:
                    projected = self.projectors[-1 * (i + 1)].fbp_channel_wise_scaled(s2)
                else:
                    channels = s2.size(1)
                    projected = self.projectors[-1 * i].fbp_channel_wise_scaled(s2)[:, :channels, :, :]

                combined_res_samples.append(torch.cat([s, projected], dim=1))
